...
NEW_FLIGHT_DATA_COMPACT_SINGLE = 'NEW_FLIGHT_DATA_COMPACT_SINGLE'

flight_data_signals = Namespace()
flight_data_signal = signal(f'{NEW_FLIGHT_DATA_COMPACT_SINGLE}')
...
async def get_or_init_flight_data_collection() -> AgnosticCollection:
    ...
    col = await get_or_init_collection(f'flight_data_compact_individual', create_collection)
    return col
...
async def insert_flight_data(measurements: list[FlightMeasurementIndividualCompactDB], flight_id: UUID):
    ...
    # s = cast(NamedSignal, signal(NEW_FLIGHT_DATA_COMPACT_SINGLE))
